    ...
    u8* ciphertext, u8 tag_out[16])
{
    ...
    J0[
        ...
        while (i + 16
            ...
            (ctr, keystream, rk);
            for (int j = 0; j < 16
                ; ++j) ciphertext[i + j] = plaintext[i + j] ^ keystream[j];
        ...
        if
            ...
            size_t
                rem = plen - i;
            for (size_t j = 0
                ; j < rem; ++j) ciphertext[i + j] = plaintext[i + j] ^ keystream[j];
            // pad block and GHASH
            memset(block, 0, 16
            );
            memcpy
            (block, ciphertext + i, rem);
            ghash_update
            (X, block, H);
        }
        ...
        u64 aadbits = (u64)aadlen *
            8
            ;
        u64 ctxtbits = (u64)plen *
            8
            ;
        for (int b = 0; b < 8; ++b) lenblock[7 - b] = (u8)(aadbits >> (8
            * b));
        for (int b = 0; b < 8; ++b) lenblock[15 - b] = (u8)(ctxtbits >> (8
            * b));
        ghash_update
        (X, lenblock, H);
        ...
        for (int k = 0; k < 8; ++k) Xbytes[k] = (u8)(X.hi >> (56 - 8
            * k));
        for (int k = 0; k < 8; ++k) Xbytes[8 + k] = (u8)(X.lo >> (56 - 8
            * k));
        for (int k = 0; k < 16
            ; ++k) tag_out[k] = Sblock[k] ^ Xbytes[k];
